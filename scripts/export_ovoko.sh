...
if [ $? -ne 0 ]; then
  echo "La tâche app:export-ovoko a échoué le $(date '+%Y-%m-%d %H:%M:%S')" | mail -s "Echec cron app:export-ovoko" florent.devynck@groupevitaminet.com,franck.depoorter@groupevitaminet.com
fi