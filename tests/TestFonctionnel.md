| **Section**               | **Critère**                                  | **État**               | **Importance** |
|---------------------------|----------------------------------------------|------------------------|----------------|
| **A. CONNEXION**                                                                                                   |
|                           | Visuel                                       | Fonctionnel            | Basse          |
|                           | Lien de connexion                            | Fonctionnel            | Haute          |
|                           | Bouton se connecter                          | Fonctionnel            | Haute          |
|                           | Bouton se déconnecter                        | OK                     | Moyenne        |
|                           | Pas de Mail                                  | Fonctionnel            | Haute          |
|                           | Pas de mot de passe                          | Fonctionnel            | Haute          |
|                           | Affichage Mot de passe                       | Fonctionnel            | Moyenne        |
|                           | Mail sans @                                  | Fonctionnel            | Haute          |
|                           | Mail sans extension                          | Fonctionnel            | Haute          |
|                           | Mauvais Mot de Passe                         | Fonctionnel            | Haute          |
|                           | Mot de passe oublié                          | A tester               | Moyenne        |
|                           | Lien Créer un compte                         | Fonctionnel            | Haute          |
| **B. INSCRIPTION**                                                                                                 |
|                           | Visuel                                       | Fonctionnel            | Basse          |
|                           | Nom Vide                                     | Fonctionnel            | Haute          |
|                           | Nom avec caractères interdits                | Erreur                 | Haute          |
|                           | Prénom Vide                                  | Fonctionnel            | Haute          |
|                           | Prénom avec caractères interdits             | Erreur                 | Haute          |
|                           | Mail Vide                                    | Fonctionnel            | Haute          |
|                           | Mail sans @                                  | Erreur                 | Haute          |
|                           | Mail sans extension                          | Erreur                 | Haute          |
|                           | Confirmation Mail Vide                       | Fonctionnel            | Haute          |
|                           | Confirmation de Mail sans @                  | Erreur                 | Haute          |
|                           | Confirmation de Mail sans extension          | Erreur                 | Haute          |
|                           | Confirmation de Mail ≠ Mail                  | Erreur                 | Haute          |
|                           | Mot de passe Vide                            | Fonctionnel            | Haute          |
|                           | Mot de passe peu de caractère                | Erreur                 | Haute          |
|                           | Mot de passe mauvaise forme                  | Erreur                 | Haute          |
|                           | Visualisation du Mot de passe                | Erreur                 | Haute          |
|                           | Confirmation Mot de passe Vide               | Fonctionnel            | Haute          |
|                           | Confirmation de Mot de passe ≠ Mot de passe  | Erreur                 | Haute          |
|                           | Confirmation Mot de passe peu de caractère   | Erreur                 | Haute          |
|                           | Confirmation Mot de passe mauvaise forme     | Erreur                 | Haute          |
|                           | Visualisation du Mot de passe                | Erreur                 | Basse          |
|                           | Menu Type de compte                          | Fonctionnel            | Haute          |
|                           | Réception de Mail de confirmation            | Erreur                 | Moyenne        |
|                           | Tester les professionnels                    | A faire                |                |
| **C. BOUTIQUE**                                                                                                    |
| 1) Entête                                                                                                          |
|                           | Logo Soluval                                 | Fonctionnel            | Moyenne        |
|                           | Se connecter                                 | OK                     | Basse          |
|                           | Bouton Panier                                | Fonctionnel            | Haute          |
|                           | 2) Type de pièce                             |                        |                |
|                           | Recherche dynamique                          | A tester               | Moyenne        |
|                           | Recherche                                    | A tester               | Haute          |
|                           | Recherche inexistante                        | A tester               | Moyenne        |
|                           | 3) Filtres                                                                             |
|                           | Marque                                       | A tester               | Haute          |
|                           | Modèle Grisée                                | A tester               | Basse          |
|                           | Modèle                                       | A tester               | Haute          |
|                           | 4) Recherche avancée                                                                   |
|                           | Visuel                                       | Fonctionnel            | Basse          |
|                           | Code Moteur                                  | A tester               | Basse          |
|                           | Code boîte                                   | A tester               | Basse          |
|                           | Référence adaptable                          | A tester               |                |
|                           | Référence constructeur                       | A tester               |                |
|                           | 5) Liste                                                                               |
|                           | Pièces trouvées                              | A tester               | Basse          |
|                           | Bouton de pagination 10                      | A tester               | Basse          |
|                           | Bouton de pagination 25                      | A tester               | Basse          |
|                           | Bouton de pagination 50                      | A tester               | Basse          |
|                           | Menu Trier par                               | A tester               | Basse          |
|                           | Menu Trier par "Date"                        | A tester               | Basse          |
|                           | Menu Trier par "Prix_Croissant"              | A tester               | Basse          |
|                           | Menu Trier par "Prix_Décroissant"            | A tester               | Basse          |
|                           | Cliquer sur l'image                          | A tester               | Moyenne        |
|                           | Description précise                          | A tester               | Basse          |
|                           | Bouton Ajout Panier                          | OK                     | Haute          |
|                           | Pagination fin de page                       | A tester               | Basse          |
|                           | Recherche Multiple                           | A tester               | Basse          |
| **D. PANIER**                                                                                                      |
|                           | Visuel                                       | Erreur                 | Basse          |
|                           | Description                                  | Fonctionnel            | Moyenne        |
|                           | Quantité                                     | A tester               | Moyenne        |
|                           | Bouton Supprimer                             | Fonctionnel            | Moyenne        |
|                           | Sous Total                                   | OK                     | Moyenne        |
|                           | Continuer vos achats                         | Fonctionnel            | Haute          |
|                           | Finaliser la commande                        | Fonctionnel            | Haute          |


(1) Veuillez remplir ce champ